...
investigate = np.where(xyzcells[:,2]<-800)
errorcellcenters = []
for i in investigate[0]:
    errorcellcenters.append(np.abs(CeCell1[i]-CeCell2[i]))
    
region = xyzcells[xyzcells[:,2]<-800] 
                            
percentage = 0.5 #Percentage of the grid you want to refine
Ncellstorefine = int(np.ceil(percentage*len(region)))
cellstorefine = region[np.argpartition(errorcellcenters,-Ncellstorefine)[-Ncellstorefine:]]
fig = plt.figure()
